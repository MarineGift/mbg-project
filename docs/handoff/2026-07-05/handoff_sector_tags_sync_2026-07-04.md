# Handoff — Sector↔TAGS 일관성 수정 (2026-07-04)

## 원인
분류가 틀린 게 아니라 **두 레이어 불일치**: Sector 필터 = `investor_sector_focus`(정규화) / TAGS 컬럼 = `parties.interest_tags`(레거시 jsonb). AM 섹터인 Scout·CTAN·Unilever 등이 레거시 태그에는 advanced_materials가 없어 화면이 어긋남.

## 수정 2건
1. **`sql/fix_sync_sector_tags.sql`** — 섹터→태그 미러링을 **양쪽 레이어에** 적용 (정규화 링크 + 레거시 jsonb append). 실행 즉시 현재 배포 UI도 일관됨. 검증: AM 섹터인데 태그 없는 투자사 = 0행(양 레이어).
2. **`patch_tags_normalized_source.ps1`** — TAGS 컬럼/정렬키가 **정규화 테이블을 소스로** 읽도록 전환 (카탈로그 sort_order 순 칩, 레거시는 백필 안 된 파티의 폴백). 이후 섹터 변경 시 sync SQL만 재실행하면 항상 일치.

## 실행 순서
1. SQL 다운로드 → `powershell -ExecutionPolicy Bypass -File C:\dev\mbg-project\tools\move-downloads.ps1` → Supabase Run → 검증 0행 확인 → **배포 UI 새로고침으로 즉시 확인 가능**
2. 아래 인라인 패치 실행 → `npm run build`
3. Finish block
...
npm run build
git status -sb
git add "src/app/(app)/[partyType]/parties/page.tsx" sql/fix_sync_sector_tags.sql sql/fix_interest_tags_curation2.sql docs/
git commit -m "fix(directory): sector<->tags consistency - sync sector focus into tags (both layers); TAGS column reads normalized source"
