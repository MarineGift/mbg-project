...
> **버전:** 2026-05-19 v2 (Phase 2 introspection 반영 — partner 매핑 정정 + module_data 키 분포 확정)
...
### 8.1 `investor_partner` 운영값 — Phase 2 introspection 으로 해결 ✅
- `app.module_type` **enum 에 `investor_partner` 없음** 확정 (9개 값 — investor, paper_mill, partner, customer, crowdfunding, product_launch, sales, filler, filler_supplier)
- 단, **`ingest.runs.module` 컬럼은 text 타입**이며 `promote_investor_partners` 함수는 `runs.module = 'investor_partner'` (text 리터럴) 를 요구함:
  ```
  IF v_run.module NOT IN ('investor_partner') THEN
      RAISE EXCEPTION 'Run "%" is for module "%". Use module=''investor_partner''.', ...
  ```
- 즉 **두 컨텍스트 분리해서 사용:**
  - `parties.module` (enum): `'investor'` 사용 — 아래 §10 참조
  - `ingest.runs.module` (text): `'investor_partner'` 사용 (partner run 시 `start_run(p_module := 'investor_partner', ...)`)
...
### 9.2 SELECT 예시 — `v_investor_outreach_list` 활용 (Phase 2 view 정의 확인 후 업데이트)

production 에 이미 `app.v_investor_outreach_list` view 존재. firm + partner JOIN 완료된 상태.
다음은 view 기반 scoring 적용 SELECT:

```sql
WITH partner_scores AS (
  SELECT 
    v.firm_id, v.firm_name, v.firm_tags, v.firm_interests,
    v.partner_id, v.partner_name, v.partner_linkedin, v.partner_email,
    v.partner_title, v.partner_seniority, v.partner_is_decision_maker,
    -- direct_fit firms (Section 3.2 의 9개) — interest_tags 또는 name 매칭
    (
      'direct_fit' = ANY(v.firm_interests)
      OR v.firm_name IN (
    ...
    v.partner_is_decision_maker::int * 25 AS s_dm,
    CASE v.partner_seniority::text
      ...
      ELSE 0
    END AS s_seniority,
    (v.partner_linkedin IS NOT NULL)::int * 5 AS s_linkedin
  FROM app.v_investor_outreach_list v
  WHERE v.partner_id IS NOT NULL  -- partner 없는 firm 행 제외
)
SELECT 
  partner_name, firm_name, partner_title, partner_seniority,
  partner_is_decision_maker, partner_linkedin, partner_email,
  (s_directfit + s_dm + s_seniority + s_linkedin) AS priority_score
FROM partner_scores
ORDER BY priority_score DESC, partner_name
LIMIT 50;
```

⚠️ **view 의 알려진 약점:** `v_investor_outreach_list` 는 partner 쪽 `deleted_at IS NULL` 필터가 누락됨 (firm 만 필터). 필요시 직접 JOIN 으로 재작성하거나 view 정의 수정 PR 권장.

⚠️ **relationship_score 컬럼은 v_investor_outreach_list 에 노출 안 됨.** 필요시 `app.parties` 직접 JOIN 으로 가져와야 함 (rubric §9.1 의 0-10 점수).
...
## 10. `module_type` 모델링 결정 (Phase 2 introspection 으로 production 검증 완료)

**확정 사항:**
- `app.module_type` enum 의 9개 값에 **`investor_partner` 없음** (06_enums.csv 직접 확인됨)
- HANDOFF.md 의 `'investor_partner'::app.module_type` 패턴은 **잘못된 표기** (enum cast 시 에러)
- 단, `ingest.runs.module` (text) 에서는 `'investor_partner'` 가 유효한 운영값 — §8.1 참조

**🔴 v1 → v2 수정 (Phase 2 production 검증으로 확인된 매핑):**
...
| **Investor side person (GP, MD 등)** | **`'investor'`** ⭐ | `'individual'` | `investor_partner_profile` |
| **일반 business partner (공급망)** | **`'partner'`** | `'company'` 또는 `'individual'` | `partner_profile` |
| Paper mill firm | `'paper_mill'` | `'organization'` | (별도 module_data) |
| Filler supplier | `'filler_supplier'` | `'organization'` | (별도 module_data) |

⚠️ **v1 의 오류:** 이전 문서는 investor side person 도 `module='partner'` 라고 잘못 표기. **실제로는 `module='investor'` + `party_type='individual'` 사용**.

**3중 production 증거:**

1. **`v_investor_outreach_list` view 정의:**
   ```sql
   LEFT JOIN app.parties p ON p.parent_party_id = f.id
                          AND p.party_type = 'individual'::app.party_type
                          AND p.module    = 'investor'::app.module_type   -- ⭐
   ```

2. **`ingest.promote_investor_partners` 의 INSERT:**
   ```sql
   INSERT INTO app.parties (..., party_type, module, parent_party_id, ...)
   VALUES (..., 'individual'::app.party_type,
                'investor'::app.module_type,  -- ⭐ NOT 'partner'
                v_firm_id, ...)
   ```

3. **module_data 키 분포 (Section 15):** `parties.module='investor'` 에 partner 속성 키 35회 출현 (title, email, linkedin, background, education, twitter, focus_areas, joined_year, role, firm_name, firm_website). 이는 firm 이 아니라 partner 의 속성.

⭐ **discriminator (구분 기준):**
- `parties.module='investor'` + `party_type='fund'/'organization'` + `parent_party_id IS NULL` → **firm** (`investor_profile` 적재)
- `parties.module='investor'` + `party_type='individual'` + `parent_party_id = <firm_id>` → **investor partner** (`investor_partner_profile` 적재)
- `parties.module='partner'` → **일반 비즈니스 partner** (공급망 등, `partner_profile` 적재)

→ `parties.module` 만으로는 investor GP vs 공급망 partner 구분 어려움. **`parent_party_id` 와 profile 테이블 존재 여부로 최종 판별**.

**현 DB 분포 (2026-05-19):** parties.module='investor' 243 = firms 111 + partners ~132 (= 119 with profile + 약간의 누락). parties.module='partner' 15 = 공급망 등.

---

## 11. `module_data jsonb` 사용 패턴 (Phase 2 Section 15 결과 반영)

5개 테이블이 `module_data jsonb DEFAULT '{}'` 보유. **현재 실제 키 분포 (parties 만 — 다른 테이블은 모두 empty):**

### 11.1 `parties.module='investor'` (partner 속성 — `party_type='individual'` 인 행)

| 키 | 사용 행 수 | 비고 |
|----|---:|------|
| source_note | 230 | enrichment 출처 텍스트 |
| ingest_run | 179 | 어느 run 으로 적재됐는지 (run_label) |
| aum_usd_billions | 106 | firm 측 (별도 — 아래 §11.2 참조) |
| founded_year | 106 | firm 측 |
| subtype / party_subtype | 55 / 53 | firm 측 |
| stage_focus_raw / sector_focus_raw | 53 / 53 | firm 측 raw 텍스트 |
| **title** | 35 | ⭐ partner 직책 |
| **email** | 35 | ⭐ partner 이메일 |
| **firm_name / firm_website** | 35 / 35 | ⭐ partner 의 firm 메타 |
| **background / education / twitter** | 35 / 35 / 35 | ⭐ partner 배경 |
| **focus_areas / role / joined_year** | 35 / 35 / 35 | ⭐ partner 속성 |
| enrichment_source | 2 | 부가 출처 |

→ partner 1명 적재 시 module_data 에 대략 12~14 키 채워짐. helper `promote_investor_partners` 가 일부 자동 처리.

### 11.2 `parties.module='paper_mill'` (industry-master 미러)

| 키 | 사용 행 수 |
|----|---:|
| main_products, auto_promoted_at, filler_use_intensity, main_product_category, evidence_level, europe_mills_footprint, industry_source, headquarters | 1055 (균일) |

→ industry schema 에서 auto-promote 된 행들이 균일한 키셋 보유.

### 11.3 `parties.module='filler'` (industry-master 미러)

| 키 | 사용 행 수 |
|----|---:|
| evidence_level, auto_promoted_at, industry_source | 362 |
| supply_model, market_role, supplier_type, onsite_pcc_evidence | 361 |
| (그 외 1행만 있는 키 5개) | 1 |

### 11.4 `app.portfolio_companies.module_data`

→ 모든 행이 `{}` (empty). 현재 미사용. 향후 co_investors / IPO target / patent count 등 용도 예약.
