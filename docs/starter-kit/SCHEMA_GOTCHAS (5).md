# SCHEMA_GOTCHAS — URM Platform 작업 함정 모음 (v3.3)

> **버전:** 2026-05-19 v3.3 (Phase 4 [A2] 완료 — §13 dedup view 활용 패턴 + GIN trgm tips + COMMENT ON syntax 함정)
...
## 12. Phase 1 (filler) 적용 후 새 함정 (v3.2, 2026-05-19)

### 12.1 ⚠️ Systemic ingest bug — 컬럼 misuse (cross-column data placement)

Phase 1 cleanup 중 발견: filler 113 rows 의 `module_data->>'supply_model'` 컬럼에 corporate URL 이 박혀 있었음 (`https://www.omya.com`, `https://www.mineralstech.com` 등 28 × 4 supplier).

원인: 이전 ingest 가 `website` 와 `supply_model` 컬럼을 혼동. spreadsheet column mapping 실수 추정.

**새 module migration 전 점검 SOP:**

```sql
-- 1. 모든 jsonb 키에서 URL/email/phone 형식 misuse scan
SELECT id, name, 
       module_data->>'<column_name>' AS suspicious_value
  FROM app.parties
 WHERE module = '<target_module>'::app.module_type 
   AND deleted_at IS NULL
   AND (module_data->>'<column_name>' LIKE 'http%' 
        OR module_data->>'<column_name>' LIKE 'www.%'
        OR module_data->>'<column_name>' LIKE '%@%.%');

-- 2. 발견 시 UPDATE 로 올바른 컬럼 (parties.website / email / phone) 으로 이동
WITH fixes AS (
  SELECT id, module_data->>'<wrong_column>' AS url
    FROM app.parties
   WHERE ...
)
UPDATE app.parties p
   SET website = u.url,
       module_data = module_data - '<wrong_column>',
       updated_at = now()
  FROM fixes u
 WHERE p.id = u.id;
```

### 12.2 🗑️ Bucket / placeholder 식별 패턴 (cleanup 가이드)

Phase 1 cleanup 으로 414 → 260 (134 buckets 제거). 식별 패턴 카탈로그:

**Cat 1: Bracket prefix bucket** — 가장 명확한 신호
- `[Other]%` — "기타" 통합 bucket (예: `[Other] Mid-tier mills`, `[Other] African cross-border`)
- `[Sector]%` — sector-level aggregate (예: `[Sector] Algeria paper`)
- `[TEST]%` — 개발 테스트 데이터 (`[TEST] MBG Holdings` 같은 self-reference 도)
- `[미등록]%` / `[Unregistered]%` — 미정의 placeholder

**Cat 2: Descriptor pseudo-bucket** — 회사명 아닌 통계 설명
- `<Region/Country> imports` / `<Region> cross-border` — "Mexican imports (Calidra/Cemex)" 등
- `Chinese gray-market` / `Chinese gray-market imports` — 회사명 X, 시장 카테고리
- `Domestic <Country> limestone` × N — 국가별 통합 설명
- `<Country>/<Country> cross-border` 패턴 — "Brazilian/Chilean cross-border (CMPC)" 등
- Multi-supplier descriptors — "Saudi/UAE/Egypt imports", "Asian imports (ASEAN)"

**Cat 3: Legacy 변형 (empty)** — 같은 firm 의 옛 이름 잔존
- `<Firm> (former <OldName> ...)` × N rows 가 모두 빈 module_data — 예: "Artemyn (former Imerys paper assets)" × 40
- 처리: original `<Firm>` 1행 유지, 변형 모두 soft-delete

**Cat 4: Near-duplicate** — 같은 회사 2 변형 (한쪽이 더 specific)
- 짧은 이름 + 긴 이름 (예: "Calidra" + "Calidra (Grupo Calidra)") → 긴 쪽 유지
- 빈 데이터 + 채워진 데이터 (예: "Arabian Cement" all NULL + "Arabian Cement Company" with data) → 채워진 쪽 유지

**처리 방법:** 모두 `UPDATE deleted_at = now() + notes = '[CLEANUP YYYY-MM-DD] Cat N reason'`. soft-delete 라 필요 시 `UPDATE deleted_at = NULL` 로 복구 가능.

### 12.3 ⚠️ Engagement-linked party 보호

Cleanup 전 항상 점검: 삭제 대상 party 가 `app.engagements`, `app.communications`, `app.investor_portfolio_companies` 등에 referenced 되는지.

```sql
SELECT 'engagements'              AS t, COUNT(*), COUNT(DISTINCT party_id)
  FROM app.engagements e
  JOIN app.parties p ON p.id = e.party_id
 WHERE p.module = '<target>'::app.module_type AND p.deleted_at IS NULL
UNION ALL ...
```

Engagement-linked 행 발견 시 (Phase 1 의 Omya (Chile) 6 engagements):
- 그 행이 진짜 회사면 → cleanup 대상 아님, 안전
- 그 행이 bucket 이면 → engagement 재할당 후 처리 또는 cleanup 보류

### 12.4 💡 3-tier hierarchy 부분 발견 — Multi-country firm 의 parent linking

Phase 1 filler 데이터에서 발견: Omya 의 country variants 20+ 가 모두 `parent_party_id = 'Omya (HQ)'` 로 linked. 즉 사용자 메모리의 "3-tier party hierarchy (group_hq → country_entity → plant)" 가 부분 적용된 상태.

**Audit query** (다른 multi-country firm 검증):
```sql
WITH multi_country_groups AS (
  SELECT TRIM(REGEXP_REPLACE(name, '\s*\(.*?\)\s*$', '', 'g')) AS root_name,
         COUNT(*) AS variants
    FROM app.parties
...
SELECT g.root_name, g.variants,
       COUNT(*) FILTER (WHERE p.parent_party_id IS NOT NULL) AS linked,
       COUNT(*) FILTER (WHERE p.parent_party_id IS NULL)     AS unlinked
  FROM multi_country_groups g
  JOIN app.parties p 
    ON TRIM(REGEXP_REPLACE(p.name, '\s*\(.*?\)\s*$', '', 'g')) = g.root_name
   AND p.module = 'filler'::app.module_type
   AND p.deleted_at IS NULL
 GROUP BY g.root_name, g.variants
 ORDER BY variants DESC;
```

→ unlinked > 0 인 group 은 Phase 4 dedup/hierarchy 작업에서 우선 처리.

### 12.5 Migration SQL — RAISE NOTICE format string 함정 ⚠️

PL/pgSQL `RAISE NOTICE` 의 format 은 sprintf-like:
- `%` — 다음 인자 consume (placeholder)
- `%%` — literal `%` 출력

자주 하는 실수: 비율 표시 시 `% (~ %%)` 에 2개 인자 전달 → "too many parameters" 컴파일 에러.

**잘못된 패턴:**
```sql
RAISE NOTICE '  - count: % (~ %%)', v_count, v_pct;
-- format 에 placeholder 1개 (%) + literal % 1개 (%%), 인자 2개 → ERROR 42601
```

**올바른 패턴:**
```sql
-- A: 2개 placeholder + literal %
RAISE NOTICE '  - count: % (~% pct)', v_count, v_pct;

-- B: 비율 빼고 count 만
RAISE NOTICE '  - count: % (out of %)', v_count, v_total;

-- C: ROUND 결과를 string concatenate
RAISE NOTICE '  - count: % (~%s)', v_count, v_pct::text || '%';
```

DO block 의 RAISE 가 실패하면 전체 트랜잭션이 ROLLBACK 됨 (BEGIN ... COMMIT 안에서). 안전성 측면에선 좋지만 작업 재실행 필요. 새 마이그레이션 작성 시 RAISE format 검증 필수.

---

## 13. ⭐ Phase 4 [A2] 적용 후 새 함정 + 활용 패턴 (v3.3, 2026-05-19)

### 13.1 ⚠️ COMMENT ON ... IS — string literal only, expression 안 됨

PostgreSQL 의 `COMMENT ON ... IS 'text'` 는 string literal 만 받음. SQL expression (concatenation `||` 등) 불가.

**잘못된 패턴 (이 세션에서 실제 발생, 트랜잭션 ROLLBACK 야기):**
```sql
COMMENT ON VIEW app.v_party_dedup_candidates IS
  'Generic dedup candidate pair list. ' ||
  'same-parent siblings + parent-child + same-root-ancestor (recursive) 모두 자동 제외. ' ||
  'A2.3 v2 (2026-05-19).';
...
**올바른 패턴:**
```sql
-- 한 줄로 합치거나, escape 없이 multiline literal
COMMENT ON VIEW app.v_party_dedup_candidates IS
  'Generic dedup candidate pair list. same-parent siblings + parent-child + same-root-ancestor (recursive) 모두 자동 제외. A2.3 v2.';

-- 또는 PostgreSQL 의 multiline literal (실제 newline 포함)
COMMENT ON VIEW app.v_party_dedup_candidates IS
  'Line 1.
Line 2.
Line 3.';
...
이 함정이 BEGIN-COMMIT 안에서 발생하면 view 생성까지 ROLLBACK 됨. 후속 SELECT 가 `relation does not exist` 에러 연발.

### 13.2 ⭐ Dedup view 사용 패턴 — 새 entity 적재 전 사전 확인

URM_MASTER §1 원칙 10 의 핵심 실현. **새 entity 적재 전 항상 dedup 사전 확인 권장**.

**Pattern A — 단발성 사전 확인:**
```sql
-- 새 firm 적재 전
SELECT name, country_code, similarity_score, match_kind
  FROM app.find_similar_parties(
    p_name      => 'Acme Minerals International',
    p_module    => 'filler'::app.module_type,
    p_threshold => 0.5
  );

-- 새 person 적재 전 (phone 도 있으면 더 정확)
SELECT name, country_code, similarity_score
  FROM app.find_similar_persons(
    p_name      => 'John Doe',
    p_phone     => '+1-512-555-0123',
    p_module    => 'investor'::app.module_type,
    p_threshold => 0.6
  );
```

**Pattern B — 주기적 dedup audit (모든 module):**
```sql
-- 상위 confidence dedup candidate
SELECT module::text, party_type::text, name_a, name_b, similarity_score, match_kind
  FROM app.v_party_dedup_candidates
 WHERE similarity_score >= 0.7
 ORDER BY similarity_score DESC, module;

-- 특정 module 의 candidate
SELECT name_a, name_b, country_a, country_b, similarity_score
  FROM app.v_party_dedup_candidates
 WHERE module = 'investor'::app.module_type
 ORDER BY similarity_score DESC;
```

**Pattern C — 적재 시 자동 dedup (ingest helper 와 결합):**
```sql
-- Ingest 함수 안에서:
-- 1. find_similar_parties 호출
-- 2. similarity_score >= 0.9 면 reject (또는 dedup_status='pending' 으로 표시)
-- 3. 0.5-0.9 면 manual review queue
-- 4. < 0.5 면 적재 진행
```

→ 자동 merge 는 **금지**. 결과는 검토 candidate 일 뿐. 동명이인 / multi-firm / false positive 가능성.

### 13.3 ⚠️ Dedup view 의 noise 발생 케이스

A2 view 의 3 단계 자동 제외에도 다음 케이스에서 noise 발생:

**케이스 1: mass-duplicate firm (parent_party_id 모두 NULL)**
- 예: Imerys 38 행이 모두 그냥 "Imerys", country_code 만 다름. parent_party_id 모두 NULL.
- 결과: 38 행 × pairwise = 703 pair 가 view 에 폭증
- → **별도 cleanup 작업 target.** view 결과 → 의도된 발견 (mass-dup 시그널)

**케이스 2: 다른 firm 이지만 이름 prefix 우연 일치**
- 예: "Heather Mack" vs "Mack Healy" — 토큰 "Mack" 공통, similarity 0.5
- → caller 가 `WHERE similarity_score >= 0.6` 또는 `WHERE match_kind = 'both'` filter

**케이스 3: paper_mill module 의 sector-bucket data**
- 1,910 pair 발생. V11.4 industry_master 의 `[Sector]` country-level bucket 들끼리 매칭.
- → V11.5 ingest 들어올 때까지 paper_mill 측 dedup audit 보류.

### 13.4 ⭐ Phone 컬럼 활용 패턴

**E.164 표시용 vs Normalized 비교용 — 분리:**
```sql
-- 적재 시: 두 컬럼 동시 채움
INSERT INTO app.parties (..., phone_e164, phone_normalized)
VALUES (..., '+1-512-555-0123', app.normalize_phone('+1-512-555-0123'));

-- 검색 시: phone_normalized 사용 (B-tree index 활용)
SELECT name FROM app.parties 
 WHERE phone_normalized = app.normalize_phone('(512) 555-0123');

-- Trigger 자동 채움 (TODO — 현재 미설정, 적재 측에서 명시적으로 set)
```

**향후 개선 후보:** `BEFORE INSERT OR UPDATE OF phone_e164` trigger 로 `phone_normalized` 자동 채움. 현재는 적재 SQL 에서 명시적 처리 필요.

### 13.5 ⚠️ GIN trgm index 의 중복 — 정리 권고

A2.1 적용 시 `parties.name_normalized` 에 이미 trigram index (`idx_parties_name_normalized`) 가 있는데 partial 신규 (`_trgm`) 를 추가해 **중복 발생**.

**현재 상태:**
```
idx_parties_name_normalized       USING gin (name_normalized gin_trgm_ops)
                                  -- WHERE 없음 (전체 row)
idx_parties_name_normalized_trgm  USING gin (name_normalized gin_trgm_ops)
                                  WHERE deleted_at IS NULL  -- partial (더 작음)
```

신규가 더 효율적 (deleted 제외). 기존 DROP 권고:
```sql
-- 별도 cleanup 작업으로
BEGIN;
...
→ 기능 영향 없음. storage 절약 + 단순화 목적.

### 13.6 ⭐ Recursive CTE depth 제한 — 무한 루프 방지

`v_party_dedup_candidates` 의 `party_roots` recursive CTE 는 `pr.depth < 5` 제한.

**현재 hierarchy 는 3-tier:** group_hq → country_entity → plant (depth 0 → 1 → 2). depth 5 면 충분.

**향후 hierarchy depth 늘리면** view 의 depth 제한도 늘려야:
```sql
-- 향후 4-tier 이상이 도입되면
WHERE pr.depth < 10  -- 또는 적절한 값
```

5 미만 cycle 이 발생하면 root 계산이 잘림 → some pair 가 noise 로 나옴. 새 hierarchy tier 도입 시 view 재정의 검토.

### 13.7 ⚠️ A2 dedup 결과 사용 시 한국 firm 정책

PROJECT_CONTEXT §2 의 "한국 firm 적재 금지" 의 **좁은 해석 적용** (이 세션 A3 결정):
- 본사가 한국인 firm 만 금지 (Hanil / Sungshin / Tongyang 등)
- 외국 본사의 KR variant 는 보존 (Imerys Korea / Omya (Korea) / Specialty Minerals (Korea))

→ dedup view 결과에서 KR variant 가 나오면 **본사 국가 확인 후 결정**. 외국 본사 자회사는 dedup 후보가 아닌 의도된 entity.
