...
> **버전:** 2026-05-19 v2.2 (Phase 2 완료 반영 — person_firm_history 119 rows + 2 views + sync trigger)
...
| 항목 | 값 |
|------|---|
| 전체 relations (app + ingest) | **85 base tables + 7 views = 92** |
| ┗ app schema | 83 base tables + 5 views |
| ┗ ingest schema | 2 base tables + 2 views |
| 전체 컬럼 | 1,560 (Phase 2 추가분 포함) |
| 전체 FK | 296 (person_firm_history FK 추가) |
| 전체 enum 타입 | **30 (app) + 1 (public) = 31** |
| Semantic CHECK 제약 (not_null 제외) | 119+ (pfh 추가분 3개) |
| 함수 (app + ingest) | **27 + 15 = 42** (sync_parent_from_primary_firm 추가) |
| Multi-tenant 테이블 (org_id 보유, FK 여부 무관) | 75/92 |
| Soft-delete 테이블 (deleted_at 보유) | 33 |

⚠️ **이전 v2 doc 의 오류 정정 (GPT-5.5 Pro 검증):** 이전에 "85 + 2 = 87 base tables" 라고 적혀 있던 것은 잘못이었습니다. `01_tables.csv` 의 app 85개는 base 82 + views 3 을 모두 포함한 숫자였습니다. 정확한 base 합계는 v2.1 기준 84개, views 5개였습니다. v2.2 (Phase 2 적용 후) 부턴 base 85개 / views 7개 (= +1 table + 2 views).

### 데이터 규모 (2026-05-19 v2.2)
...
| `app.person_firm_history` ⭐ NEW | **119** |
