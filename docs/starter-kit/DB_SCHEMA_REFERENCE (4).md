...
> **버전:** 2026-05-19 v2.4 (Phase 4 [A2] 완료 — activity_status enum + parties phone 3 컬럼 + dedup 함수 3개 + dedup view 2개. A3 filler 추가 cleanup (260→235) 반영.)
...
- v2.3 → v2.4 (A3 + A2 적용): A3 filler 추가 cleanup (260 → 235) + Phase 4 [A2] dedup 인프라 (enum 32, function 45, view 10)
- ⭐ **partner 매핑 정정:** investor side person 은 `module='investor'` + `party_type='individual'` (이전 표기 `module='partner'` 는 오류 — §3 `app.parties` 카드 참조)
- ⭐ **`module='filler_supplier'` 미사용 확정:** enum 에만 있고 0 row. 모든 filler 데이터는 `module='filler'`.
...
| 항목 | 값 |
|------|---|
| 전체 relations (app + ingest) | **87 base tables + 10 views = 97** |
| ┗ app schema | 85 base tables + 8 views |
| ┗ ingest schema | 2 base tables + 2 views |
| 전체 컬럼 | 1,608 (parties 의 A2 컬럼 3개 추가) |
| 전체 FK | 302 |
| 전체 enum 타입 | **31 (app, +activity_status) + 1 (public) = 32** |
| Semantic CHECK 제약 (not_null 제외) | 124+ |
| 함수 (app + ingest) | **30 (app, +normalize_phone/find_similar_parties/find_similar_persons) + 15 (ingest) = 45** |
| Multi-tenant 테이블 (org_id 보유) | 77/97 |
| Soft-delete 테이블 (deleted_at 보유) | 35 |

⚠️ **이전 v2 doc 의 오류 정정 (GPT-5.5 Pro 검증):** 이전에 "85 + 2 = 87 base tables" 라고 적혀 있던 것은 잘못이었습니다. `01_tables.csv` 의 app 85개는 base 82 + views 3 을 모두 포함한 숫자였습니다. 정확한 base 합계는 v2.1 기준 84개, views 5개였습니다. v2.2 부터 Phase 2 가 +1 base (person_firm_history) + 2 views 추가. v2.3 부터 Phase 1 filler 가 +2 base + 1 view 추가. v2.4 부터 Phase 4 [A2] 가 +2 views (v_party_dedup_candidates, v_person_dedup_candidates) 추가.
...
| Schema | Enum | 허용 값 |
|--------|------|---------|
| `app` | `activity_status` ⭐ A2 | active, on_leave, retired, deceased, unknown |
...
**중요 enum 사용처:**
- `app.module_type` — 10 컬럼에서 사용. 모든 cross-domain 통합의 기반. (실사용: `'filler_supplier'` 값은 enum 에만 있고 0 row)
...
- `app.activity_status` ⭐ A2 — 1 컬럼. `parties.activity_status` (active/on_leave/retired/deceased/unknown). entity_status (active/inactive/archived/blocked) 와 직교.
...
### `app.parties`  *(rows: 1,739 — A3 net cleanup 후. active 1,552 / soft_deleted 187)*
...
| 42 | `activity_status` | activity_status |  |  ⭐ A2 |
| 43 | `phone_e164` | text |  |  ⭐ A2 |
| 44 | `phone_normalized` | text |  |  ⭐ A2 |
...
### `app.filler_supplier_profile`  *(rows: 235)* ⭐ Phase 1 (2026-05-19 적용) + A3 orphan cleanup (260 → 235)
...
#### ⭐ A2 dedup 함수 3개 (2026-05-19, Phase 4)

```sql
-- normalize_phone(text) → text  (IMMUTABLE)
...
-- find_similar_parties(...) → TABLE (STABLE, 7 args)
-- name trigram similarity (≥ threshold) OR phone exact match
SELECT * FROM app.find_similar_parties(
  p_name        => 'New Company Name',
  p_phone       => NULL,                         -- optional
  p_party_type  => NULL,                         -- 'company' / 'individual' / ...
  p_module      => 'filler'::app.module_type,    -- optional filter
  p_exclude_id  => NULL,                         -- 자기 자신 제외 시
  p_threshold   => 0.5,                          -- default 0.3
  p_limit       => 20
);
-- 반환 컬럼: id, name, party_type, module, country_code, parent_party_id, similarity_score, match_kind ('name_trgm'|'phone_exact'|'both')

-- find_similar_persons(...) → TABLE (STABLE, 6 args, wrapper)
-- party_type='individual' 강제 wrapper
...
**Acceptance test 결과 (filler module):**
- "EGM Wierzbica" vs "EGM (Wierzbica)" — similarity 1.00 ✓
- "Quality Minerals Public Co. (Q-min)" vs "...Public Co., Ltd. (Q-min)" — 0.88 ✓
- "Mikron-S Mikronize (TURKCARB)" vs "...Mineral (TURKCARB)" — 0.79 ✓

---

## 4.1 ⭐ Dedup View 카드 (2개, A2 Phase 4)

### `app.v_party_dedup_candidates` *(view)*

**역할:** Generic dedup candidate pair list. self-join + trigram + phone exact. **3 단계 자동 제외**:
1. same-parent siblings (같은 firm 의 country variants — Specialty Minerals (USA) ↔ (Korea))
2. 직접 hierarchy (parent-child — Omya (HQ) ↔ Omya (USA))
3. **same-root-ancestor (recursive CTE)** — Specialty Minerals 95 variants 끼리 모두 자동 제외

**반환 컬럼 (12개):**
| column | 설명 |
|--------|------|
| `party_a` (uuid) | LEAST(a.id, b.id) — 정렬된 pair 의 첫 번째 |
| `party_b` (uuid) | GREATEST(a.id, b.id) |
| `name_a`, `name_b` (text) | 정렬 기준에 맞는 이름 |
| `country_a`, `country_b` (text) | 국가 코드 |
| `party_type` (party_type) | 같은 type 만 매칭 (개체끼리 비교) |
| `module` (module_type) | 같은 module 만 매칭 |
| `similarity_score` (real) | GREATEST(name_trgm, phone_exact). 0-1 범위. |
| `match_kind` (text) | 'name_trgm' / 'phone_exact' / 'both' |
| `a_parent_id`, `b_parent_id` (uuid) | 진단용 — parent_party_id 노출 |
| `a_root_id`, `b_root_id` (uuid) | 진단용 — recursive root |

**WHERE 조건 (view 내부):**
- `a.id < b.id` — pair 한 방향만 keep (중복 방지)
- `a.deleted_at IS NULL AND b.deleted_at IS NULL`
- `a.party_type = b.party_type AND a.module = b.module` — type/module 같아야
- `(name_normalized % name_normalized AND similarity >= 0.5) OR (phone_normalized = phone_normalized)`
- 3 단계 제외 (위 1-3)

**Threshold:** view 안 hard floor `0.5`. caller 가 추가 filter 가능 (`WHERE similarity_score >= 0.7`).

**사용 패턴:**
```sql
-- 모든 dedup candidate top 20
...
-- 특정 module
SELECT * FROM app.v_party_dedup_candidates 
 WHERE module = 'filler'::app.module_type 
 ORDER BY similarity_score DESC;

-- High-confidence dedup 만 (자동 merge 후보)
SELECT * FROM app.v_party_dedup_candidates 
 WHERE similarity_score >= 0.9
   AND match_kind IN ('both','phone_exact');
```

### `app.v_person_dedup_candidates` *(view, wrapper)*

**역할:** `v_party_dedup_candidates` 의 `party_type='individual'` filter wrapper. URM_MASTER §1 원칙 10.
...
**현재 결과 (3 pair):**
- Brian Smith × 2 (US/US, 1.00) — 동명이인 (LinkedIn 다름)
- Lior Susan × 2 (US/US, 1.00) — multi-firm 가능성 (LinkedIn 검증 후 merge 결정)
- Heather Mack ↔ Mack Healy (US/US, 0.50) — false positive

→ **자동 merge 금지.** 결과는 사람 검토 candidate 리스트.
