...
> **버전:** 2026-05-19 v3 (Phase 1 부분 완료 — filler_supplier_profile 260 rows + cleanup 134 buckets. paper_mill 보류.)
...
| **Paper mill profile (firm+person)** | 🟡 보류 — 1,070 rows 가 V11.4 industry_master 의 sector-level bucket. 도메인 데이터 99% NULL | 미래 ingest 시 적용 | **데이터 부재로 영구 보류 가능** |
| **Filler supplier profile (firm+person)** | ✅ 적용됨 (260 rows after cleanup) + view `v_filler_suppliers` + 3-tier hierarchy 부분 발견 | ✅ | — |
...
### 🟢 Phase 1 (filler) — 완료 (2026-05-19)

- ✅ **Cleanup 414 → 260 rows** — 134 행 soft-delete:
  - Cat 1 bracket buckets (`[Other]%`, `[Sector]%`, `[TEST]%`, `[미등록]%`) 25개
  - Cat 2 descriptor pseudo-buckets (Chinese gray-market, Domestic X limestone, imports 류) ~62개
  - Cat 3 Artemyn legacy duplicates 40개 (모두 empty)
  - Cat 4 near-duplicate consolidation 8개 (Arabian Cement → Arabian Cement Company 등)
- ✅ **Systemic ingest bug 봉합** — `supply_model` 컬럼에 박혀 있던 corporate URL (113 rows) → `parties.website` 이전
- ✅ **`filler_supplier_profile`** (firm-level, 260 rows): supplier_type / market_role / supply_model / onsite_pcc_evidence + 3 메타
- ✅ **`filler_supplier_contact_profile`** (person-level, 0 rows, schema-only)
- ✅ **View `v_filler_suppliers`** — firm + profile + parent_party_id 통합, `has_full_supplier_data` flag
- ✅ **RLS 8 policies + 2 trigger** (set_updated_at)
- 💡 **3-tier hierarchy 부분 발견** — Omya 의 country_entity 20+ 행 모두 "Omya (HQ)" 로 parent linking 이미 완료. 다른 multi-country firm 도 동일한지 audit 필요.

### 🟡 Phase 1 (paper_mill) — 보류 (데이터 부재)

- 1,070 paper_mill rows 중 거의 전부 V11.4 industry_master 의 `[Sector]` country-level 버킷
- 8개 도메인 키 (`main_product_category`, `headquarters`, `main_products` 등) 모두 schema 만 있고 값 99% NULL
- profile 로 승격할 가치 없음 — V11.5 ingest 또는 별도 mill-level 데이터 들어올 때까지 동결
- 동결 결정 (사용자 합의 2026-05-19): "검색 가능한 entity 만 의미. 데이터 부재 module 은 노력 낭비."

### ✅ Phase 1 — filler 부분 완료 [DONE 2026-05-19] / paper_mill 보류
이전 위치에 있던 Phase 1 의 SQL/산출물은 위 §6 "Phase 1 (filler)" 섹션에 통합. paper_mill 은 데이터 부재로 동결.
...
| v3 | 2026-05-19 | Phase 1 filler 완료 + paper_mill 동결 — §5 Gap 표 + §6 Phase 1 (filler) ✅ + Phase 1 (paper_mill) 🟡 보류 + cleanup 인사이트 (bucket / supply_model URL leak / 3-tier hierarchy 부분 발견) |

---

## 12. 다음 액션 (현재 시점)

✅ **Phase 0 + 1 (filler) + 2 완료** = URM 통일 패턴의 Layer 2 (filler profile) + Layer 3 (temporal) 완성.
→ 다음: **Phase 4 (activity_status + phone_e164 + find_similar_persons)** — dedup 인프라.

이유:
- Phase 1 진행 중 발견한 multi-country variants (Specialty Minerals 95, Omya 39, Imerys 38 등) 가 dedup 작업의 대상.
- 3-tier hierarchy (Omya 예시) 가 부분 적용된 상태 — Phase 4 에서 audit + 완성 가능.
- Phase 1 paper_mill 은 데이터 부재로 동결 → 다음 큰 작업 후보가 Phase 4.

대안:
- Phase 3 (engagement_participants) — engagement 30개의 firm/person 분리 모델.
- Outreach 분석 — Top 30 시드 CSV status 채워졌으면 응답률 분석.
- 3-tier hierarchy audit — Specialty Minerals / Imerys / Schaefer Kalk 의 parent_party_id 완비 여부 확인 (Phase 4 곁가지).
