...
| `person_firm_history` | 🟡 설계됨, 미적용 | ✅ | Phase 2 |
...
### 🔵 Phase 2 — person_firm_history (이력 추적)
**목적:** 사용자 원칙 4, 7, 9 실현.

**산출물:** `migration_person_firm_history_2026Q2.sql` (이미 작성됨, 실행 대기).
...
✅ **이 문서 작성 완료** = vision 단일 source 확보.
→ 다음: 어느 Phase 부터 실행할지 결정.

권장 시작: **Phase 2 (person_firm_history) 먼저 실행** — SQL 이미 작성됨, 5분 안에 적용 가능, 즉시 가치.
그 후 Phase 1 (paper_mill/filler profile) — 큰 작업이지만 통일 패턴 시현.
그 다음 Phase 4 / 3 / 5 / 6 순서로.
