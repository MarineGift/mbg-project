...
const LEVEL_NAMES: Record<number, string> = {
  1: 'Nasdaq Legal Eligibility — can we list?',
  2: 'Audit & Financial Reporting — can we file with the SEC?',
  3: 'FCC Commercialization — is there a business worth listing?',
  4: 'IP & Legal — do we own and protect the royalty stream?',
  5: 'Governance — can we operate as a public company?',
  6: 'Capital Markets — will underwriters take it and investors buy?',
};
    ...
    <div className="border-b py-2">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {g.pass_condition && <div className="text-xs text-muted-foreground">Pass condition: {g.pass_condition}</div>}
          <div className="text-xs text-muted-foreground">
            Linked milestones {g.linked_done}/{g.linked_total} done
            {g.assessed_at && <> · assessed {g.assessed_at}</>}
            {g.evidence && !open && <> · evidence: {g.evidence}</>}
          </div>
        </div>
        <button type="button" className="shrink-0 rounded border px-2 py-1 text-xs" onClick={() => setOpen(!open)}>{open ? 'Close' : 'Assess'}</button>
      </div>
      {open && (
        <div className="mt-2 flex flex-col gap-2 pl-14">
          ...
          <textarea value={evidence} onChange={(e) => setEvidence(e.target.value)} rows={2}
            placeholder="What did you look at? (document, figure, date)" className="rounded border bg-background p-2 text-sm" />
          <div className="flex items-center gap-2">
            <button type="button" disabled={pending}
              ...
              className="rounded bg-foreground px-3 py-1 text-xs text-background disabled:opacity-50">Save</button>
