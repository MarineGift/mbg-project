-- ==========================================================================
-- backfill_20260921b_investor_followups.sql
--
...
--
--   D  (new) subject names an investor firm, e.g. Intro Meeting | MarineBio
--      Group <> Strategic Ventures, and exactly one investor party has it
--   E  (new) same sender already has mail tagged Investors (the intro reply)
--      so later mail (calendar invites, follow-ups) follows it to that firm
--   D and E touch only unlinked or mentor-linked, non-automated mail.
--   Superset of backfill_20260921_greentown_investor_intro.sql - run this one.
--
...
-- 3) rule D - investor firm named in the subject
with c0 as (
  select c.id, c.organization_id, lower(coalesce(c.subject, '')) as subj
  from app.communications c
  left join app.parties p on p.id = c.party_id
  left join app.party_types pt on pt.id = p.party_type_id
  where c.direction = 'inbound'
...
c1 as (
  select id, organization_id,
         btrim(regexp_replace(regexp_replace(coalesce(
           substring(subj from '(?:marine\s*bio(?:\s*group)?(?:,?\s*inc\.?)?|mbg)\s*(?:<>|<->)\s*([^|\n@()]+)'),
           substring(subj from '([^|\n@()<>]+?)\s*(?:<>|<->)\s*(?:marine\s*bio|mbg)')
         ), '\s+@.*$', ''), '[\s.!?|,]+$', '')) as firm
  from c0
),
m as (
...
-- 4) rule E - sender already has mail tagged Investors
with known as (
  select distinct on (lower(c.from_address))
         lower(c.from_address) as sender, c.organization_id, c.party_id,
  ...
  where c.direction = 'inbound'
    ...
    and coalesce(c.from_address, '') <> ''
  order by lower(c.from_address), c.occurred_at desc
),
t as (
  select c.id, known.party_id as investor_party,
         (known.party_code = 'investor') as can_relink
  from app.communications c
  join known
    on known.sender = lower(c.from_address)
   and known.organization_id = c.organization_id
  left join app.parties p on p.id = c.party_id
  left join app.party_types pt on pt.id = p.party_type_id
  where c.direction = 'inbound'
    and c.deleted_at is null
...
update app.communications c
set party_id = case when t.can_relink then t.investor_party else c.party_id end,
    contact_id = case
      when not t.can_relink then c.contact_id
      when exists (select 1 from app.contacts k
                   where k.id = c.contact_id and k.party_id = t.investor_party)
        then c.contact_id
      else null
    end,
    external_data = coalesce(c.external_data, '{}'::jsonb)
      || jsonb_build_object(
           ...
           'investor_intro', jsonb_build_object(
             ...
             'firm_hint', null,
             'relinked', coalesce(t.can_relink, false),
             'backfilled', true))
from t
where c.id = t.id;

-- 5) result (the only grid the editor shows)
